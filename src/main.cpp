...
void setup() {
  ...
  server.on("/mode1", HTTP_POST, []() {       
    clockMode1 = 1;     
    server.send(200, "text/json", "\result\":\"Mode 1 activated\"}");
  });

  server.on("/mode2", HTTP_POST, []() {       
    clockMode1 = 2;     
    server.send(200, "text/json", "{\"result\":\"Mode 2 activated\"}");
  });
  server.on("/mode3", HTTP_POST, []() {       
    clockMode1 = 3;     
    server.send(200, "text/json", "{\"result\":\"Mode 3 activated\"}");
  });
  server.on("/mode4", HTTP_POST, []() {       
    clockMode1 = 4;     
    server.send(200, "text/json", "{\"result\":\"Mode 4 activated\"}");
  });
...
void loop(){
  ...
  if (currentMillis - prevTime >= 1000) {
    ...
    if(clockMode1 == 1){
      ...
      Serial1.println("A");
      // clockMode1 = 0;
    }else if(clockMode1 == 2){
      // Serial.println("B");
      Serial1.println("B");
      // clockMode1 = 0;
    }else if(clockMode1 == 3){
      Serial.println("C");
      Serial1.println("C");
      clockMode1 = 0;
    }else if(clockMode1 == 4){
      Serial.println("D");
      Serial1.println("D");
      clockMode1 = 0;
    }
